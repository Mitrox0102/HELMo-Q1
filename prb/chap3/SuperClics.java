...
public class SuperClics {
	...
	private void initialiser() {
        int xCercle = (int) (Math.random() * 600);
        int yCercle = (int) (Math.random() * 600);
		dessinerCercle(xCercle, yCercle, 4);
        
        enregistrerEntier("xCercle", xCercle);
        enregistrerEntier("yCercle", yCercle);
	...
	private void clicSouris(int xClic, int yClic) {
        ...
        int xCercle = lireEntier("xCercle");
        int yCercle = lireEntier("yCercle");
        double distPointClic = Math.sqrt(Math.pow((xClic - xCercle), 2) + Math.pow((yClic - yCercle), 2));
        
        double dureeEnSec = Chronometre.dureeEcoulee() / 1000.0;
        
        if (distPointClic == 0.0){
            distPointClic = 0.000000000000001;
        }
        
        double score = (100) / (dureeEnSec * distPointClic);
        
        System.out.println(score);
        Chronometre.demarrer();
        
        System.out.println(distPointClic);
	}
